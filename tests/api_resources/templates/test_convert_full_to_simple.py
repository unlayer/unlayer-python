# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from unlayer import Unlayer, AsyncUnlayer
from tests.utils import assert_matches_type
from unlayer.types.templates import ConvertFullToSimpleCreateResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestConvertFullToSimple:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: Unlayer) -> None:
        convert_full_to_simple = client.templates.convert_full_to_simple.create(
            design={"body": {"foo": "bar"}},
        )
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: Unlayer) -> None:
        convert_full_to_simple = client.templates.convert_full_to_simple.create(
            design={
                "body": {"foo": "bar"},
                "counters": {"foo": "bar"},
                "schema_version": 0,
            },
            display_mode="email",
            include_conversion=True,
            include_default_values=True,
        )
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: Unlayer) -> None:
        response = client.templates.convert_full_to_simple.with_raw_response.create(
            design={"body": {"foo": "bar"}},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        convert_full_to_simple = response.parse()
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Unlayer) -> None:
        with client.templates.convert_full_to_simple.with_streaming_response.create(
            design={"body": {"foo": "bar"}},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            convert_full_to_simple = response.parse()
            assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncConvertFullToSimple:
    parametrize = pytest.mark.parametrize(
        "async_client", [False, True, {"http_client": "aiohttp"}], indirect=True, ids=["loose", "strict", "aiohttp"]
    )

    @parametrize
    async def test_method_create(self, async_client: AsyncUnlayer) -> None:
        convert_full_to_simple = await async_client.templates.convert_full_to_simple.create(
            design={"body": {"foo": "bar"}},
        )
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncUnlayer) -> None:
        convert_full_to_simple = await async_client.templates.convert_full_to_simple.create(
            design={
                "body": {"foo": "bar"},
                "counters": {"foo": "bar"},
                "schema_version": 0,
            },
            display_mode="email",
            include_conversion=True,
            include_default_values=True,
        )
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncUnlayer) -> None:
        response = await async_client.templates.convert_full_to_simple.with_raw_response.create(
            design={"body": {"foo": "bar"}},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        convert_full_to_simple = await response.parse()
        assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncUnlayer) -> None:
        async with async_client.templates.convert_full_to_simple.with_streaming_response.create(
            design={"body": {"foo": "bar"}},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            convert_full_to_simple = await response.parse()
            assert_matches_type(ConvertFullToSimpleCreateResponse, convert_full_to_simple, path=["response"])

        assert cast(Any, response.is_closed) is True
