...
class TestExportImage:
    ...
    def test_method_create(self, client: Unlayer) -> None:
        export_image = client.templates.export_image.create(
            design={},
        )
        assert_matches_type(ExportImageCreateResponse, export_image, path=["response"])
    ...
    def test_raw_response_create(self, client: Unlayer) -> None:
        response = client.templates.export_image.with_raw_response.create(
            design={},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        export_image = response.parse()
        assert_matches_type(ExportImageCreateResponse, export_image, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Unlayer) -> None:
        with client.templates.export_image.with_streaming_response.create(
            design={},
        ) as response:
...
class TestAsyncExportImage:
    ...
    async def test_method_create(self, async_client: AsyncUnlayer) -> None:
        export_image = await async_client.templates.export_image.create(
            design={},
        )
        assert_matches_type(ExportImageCreateResponse, export_image, path=["response"])
    ...
    async def test_raw_response_create(self, async_client: AsyncUnlayer) -> None:
        response = await async_client.templates.export_image.with_raw_response.create(
            design={},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        export_image = await response.parse()
        assert_matches_type(ExportImageCreateResponse, export_image, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncUnlayer) -> None:
        async with async_client.templates.export_image.with_streaming_response.create(
            design={},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
